...
@Composable
fun LazyColumnExample(context: Context) {
...
@Composable
fun DefaultPreview() {
    LazyColumnExample(LocalContext.current)
}

fun getData(context: Context) : List<Movie> {
    val response = getJsonDataFromAsset(context, "response.json")
    val gson = Gson()
